...
-- 最初に指定した桁数を超えてデータを格納したときはまるめられて、指定した桁数になる
...
-- DATETIME, TIMESTAMP ()で小数点を扱うことができる指定より大きい桁は四捨五入
...
-- TIMESTAMP型は、1970-01-01から2038-01-17まで
-- TIMESTAMP型のほうが容量はすくない。created_atやupdate_atはTIMESTAMPがよい。（デバッグ目的にしか使わない、すべてのテーブルに必要）
...
-- nameにindexを貼っても、LOWER(name)に対してindexは貼られないので、関数インデックスを貼る必要がある。
